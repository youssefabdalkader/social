        ...
        <div class="max-w-7xl mx-auto flex justify-between items-center p-3">
            ...
            <div class="flex items-center gap-3">
                ...
                @if (!(auth()->user()->hasRole('User') && auth()->user()->roles->count() === 1))
                    <a href="{{ url('/admin') }}"
                        class="primary inline-block px-6 py-3 rounded-lg bg-blue-600 text-white font-semibold shadow-md hover:bg-blue-700 transition duration-300">
                        Go To Dashboard
                    </a>
                @endif
        ...
        <div class="w-1/4 space-y-4">
            ...
            <div class="bg-white p-4 rounded-xl shadow text-center">
                ...
                <div class="flex flex-col gap-2 text-sm">
                    ...
                    <a href="{{ route('posts.index') }}" class="bg-blue-500 text-white py-1 rounded text-center">
                        🏠 Home
                    </a>

                    <a href="{{ route('profile') }}" class="bg-gray-200 py-1 rounded text-center">
                        📄 My Posts
                    </a>
        ...
        <div class="w-2/4">
            ...
            <div class="bg-white p-4 rounded-xl shadow-sm mb-5">
                <form method="POST" action="{{ route('posts.store') }}" enctype="multipart/form-data">
                    ...
                    <textarea name="content" class="w-full border p-2 rounded text-sm" placeholder="Share something..."></textarea>
            ...
            @foreach ($posts as $post)
                ...
                <div id="post-{{ $post->id }}" class="bg-white rounded-xl shadow-sm mb-5 p-4">
                    ...
                    @if ($post->image)
                        <img src="{{ asset('storage/' . $post->image) }}" class="mt-3 rounded-lg max-h-80 mx-auto">
                    @endif
                    ...
                    <div id="comments-{{ $post->id }}" class="hidden mt-3">
                        ...
                        <form method="POST"
                            action="{{ route('comments.store', $post->id) }}#post-{{ $post->id }}">
                            @csrf
                            <input name="content" class="w-full border p-2 rounded mt-2 text-sm"
